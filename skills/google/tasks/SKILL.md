...
(`node /app/.pi/skills/google/tasks/scripts/tasks.mjs`), which uses the shared Google OAuth token.
...
```bash
node /app/.pi/skills/google/tasks/scripts/tasks.mjs add "Pick up parcel"                       # → Todo (default)
node /app/.pi/skills/google/tasks/scripts/tasks.mjs add "Call dentist" --due 2026-06-20         # → Todo, with a due date
node /app/.pi/skills/google/tasks/scripts/tasks.mjs add "Milch" --list "Einkaufsliste"          # → shopping list
node /app/.pi/skills/google/tasks/scripts/tasks.mjs add "Renew passport (from scan.pdf)" --list "Inbox"  # → capture
```
`--due` is `YYYY-MM-DD`. Confirm what you added **and to which list**.

## List / change tasks
```bash
node /app/.pi/skills/google/tasks/scripts/tasks.mjs list                       # open tasks on Todo (default)
node /app/.pi/skills/google/tasks/scripts/tasks.mjs list --list "Einkaufsliste" # open tasks on another list
...
node /app/.pi/skills/google/tasks/scripts/tasks.mjs lists                      # show all task lists
```
