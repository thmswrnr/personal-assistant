...
node /app/.pi/skills/google/gmail/scripts/gmail.mjs search "is:unread newer_than:2d" 10

# Read one message in full (use an id from search results)
node /app/.pi/skills/google/gmail/scripts/gmail.mjs read <messageId>

# List labels
node /app/.pi/skills/google/gmail/scripts/gmail.mjs labels

# Create a DRAFT (saved to Gmail, not sent — see "Sending" below for the approval flow)
node /app/.pi/skills/google/gmail/scripts/gmail.mjs draft --to "alice@example.com" --subject "Re: lunch" --body "Sounds good — see you at noon."

# SEND a draft the user has reviewed and explicitly approved (uses the draftId from `draft`)
node /app/.pi/skills/google/gmail/scripts/gmail.mjs send <draftId>

# MODIFY labels — mark read/unread, archive, star (only when the user asks)
node /app/.pi/skills/google/gmail/scripts/gmail.mjs modify <messageId> --read
node /app/.pi/skills/google/gmail/scripts/gmail.mjs modify <messageId> --archive
node /app/.pi/skills/google/gmail/scripts/gmail.mjs modify <messageId> --add STARRED --remove UNREAD
```
